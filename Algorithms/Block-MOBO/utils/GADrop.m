function Offspring = GADrop(Parent,options,Parameter)
...
% [5] Ye Tian, Ran Cheng, Xingyi Zhang, and Yaochu Jin, PlatEMO: A MATLAB 
...
%--------------------------------------------------------------------------
    ...
    switch Global.encoding
        ...
        otherwise
            ...
            % Polynomial mutation
            Lower = repmat(Global.lower(:,options.subidx),2*N,1);
            Upper = repmat(Global.upper(:,options.subidx),2*N,1);
            Site  = rand(2*N,D) < proM/D;
            mu    = rand(2*N,D);
            temp  = Site & mu<=0.5;
